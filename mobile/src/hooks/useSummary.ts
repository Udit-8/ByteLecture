...
import { summaryAPI, GenerateSummaryRequest, Summary, SummaryOptions, CacheStats } from '../services/summaryAPI';
...
interface UseSummaryActions {
  ...
  quickSummary: (content: string, contentType: 'pdf' | 'youtube' | 'audio' | 'text', length?: 'short' | 'medium' | 'long') => Promise<Summary | null>;
...
export const useSummary = (): UseSummaryReturn => {
  ...
  const generateSummary = useCallback(async (request: GenerateSummaryRequest): Promise<Summary | null> => {
    try {
      setLoading(true);
      setError(null);
  ...
  const quickSummary = useCallback(async (
    content: string,
  ...
  const getUserSummaries = useCallback(async (params?: {
    contentType?: string;
    limit?: number;
    offset?: number;
    sortBy?: 'created_at' | 'last_accessed_at' | 'access_count';
  ...
  const getSummariesByContentItem = useCallback(async (contentItemId: string): Promise<Summary[]> => {
    try {
      setLoading(true);
      setError(null);

      const response = await summaryAPI.getSummariesByContentItem(contentItemId);

      if (response.success && response.summaries) {
        setSummaries(response.summaries);
        return response.summaries;
      } else {
        setError(response.error || 'Failed to get summaries for content item');
        return [];
      }
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : 'Unexpected error occurred';
  ...
  const refreshCacheStats = useCallback(async (): Promise<CacheStats | null> => {
    try {
      setError(null);
