  ...
  storage: {
    from: (bucket) => ({
      upload: async (filePath, file, options) => {
        console.log(
          `📤 Mock upload to bucket '${bucket}' at path '${filePath}'`
        );
...
class UploadServiceTester {
    ...
    return {
      ...
      async uploadWithProgress(file, onProgress) {
        ...
        for (let i = 0; i < totalChunks; i++) {
          ...
          console.log(
            `📈 Progress: ${Math.round(progress)}% (${Math.round(bytesTransferred / 1024)}KB)`
          );
  ...
  async testFileValidation() {
    ...
    for (const testCase of testCases) {
      ...
      console.log(
        `${passed ? '✅' : '❌'} Expected: ${testCase.shouldPass}, Got: ${result.isValid}`
      );
  ...
  async testProgressUpload() {
    ...
    try {
      const result = await this.uploadService.uploadWithProgress(
        testFile,
        progressCallback
      );
