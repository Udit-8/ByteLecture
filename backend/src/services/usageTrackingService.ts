...
class UsageTrackingService {
   ...
   */
  async getPlanLimits(planType: 'free' | 'premium' | 'enterprise'): Promise<PlanLimit[]> {
    try {
   ...
   */
  async canUploadPDF(userId: string): Promise<{ allowed: boolean; reason?: string; quota?: QuotaResult }> {
    try {
   ...
   */
  async recordPDFUpload(userId: string): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  formatQuotaErrorMessage(quota: QuotaResult): string {
    ...
    return `You've reached your daily limit of ${quota.daily_limit} uploads. ` +
